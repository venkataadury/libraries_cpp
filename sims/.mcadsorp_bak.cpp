...
double EX=1;
const double k=1.38e-3,T=300;
const double LAMS=1.38e-3*1.93,VS=33.49,V=243.6,a=pow(sqrt(2)*V,1/3.0);
const double ETAS=1.38e-3*148.2,SIGMA=3.817,CUTOFF=0;//SIGMA/20;
...
{/*cout << EX*(4*ETAS*(pow(SIGMA/d,12)-pow(SIGMA/d,6))) << ",";*/ return ((d<=CUTOFF)?1e9:0)+EX*(4*ETAS*(pow(SIGMA/d,12)-pow(SIGMA/d,6)));}
...
int main()
{
  ...
  for(int I=0;I<=it;I++)
  {
    ...
    {
      e=energy(randomConfig(n));
      if(e<0)
        energies.push_back(e);
      //cout <<I<<"; "<< e << "\n";
    }
    f=((double)energies.size())/nmoves;
    //cout <<I<<"\t"<< stats::mean(energies) <<"\t"<<f<<"\n";
    e=pow(1.3333333333*3.141592*a*a*a,n)*f;
    Z0.push_back(e);
